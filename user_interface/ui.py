...
class Button:
    def __init__(self, text, position = (width-230, 400) , w = 100, h= 50, border=0, color = (30, 30, 30), borderColor = (0, 0, 0)):
        self.text = text
...
class Panel:
    def __init__(self, position = (width-400, 90), w= 385, h= 800, color=(60, 60, 65), alpha=100):
        self.position = position
        self.w = w
        self.h = h
        self.color = color
...
class DropDownButton:
    ...
    def updateChildren(self, value):
        ...
        for i in range(self.children_size):
            button = Button("button " +str(i), (self.position[0], self.position[1] + self.h + self.h*i+  2), self.w, self.h, 0, self.border, self.color, self.borderColor)
            self.childs.append(button)
